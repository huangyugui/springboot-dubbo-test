 ...
 * @version 1.0
 */
public class CreateSessionSampleCallBack {
    ...
    public static void main(String[] args) throws Exception {
        ...
        curator.create().inBackground((client, event) -> {
        ...
        //临时节点
        curator.create().withMode(CreateMode.EPHEMERAL).inBackground((client, event) -> {
            System.out.println("callback=============" + event);
            //异步回调都是由EventThread这个线程来执行的，串行处理保证其顺序性
            System.out.println(Thread.currentThread().getName());
            cdl.countDown();
        }).forPath("/a/b", "/a/b".getBytes());
