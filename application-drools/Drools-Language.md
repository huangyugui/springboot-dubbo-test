[toc]
## Drools语法-Language
[Drools7.0.0.Final规则引擎教程](http://blog.csdn.net/wo541075754/article/category/6397798/2)
### 关键词
...
#### 属性部分-Attributes
  ...
  - no-loop
      ...
      no-loop表示执行一次之后就不在执行了，即使了有规则执行了insert、update、retract，只要执行了一次，就不会导致第二次执行
      但是lock-on-active 和no-loop的不同在于，即使了有规则执行了insert、update、retract，即使一次也没执行，也不会导致执行，但是java代码insert进来的，并不是规则内部修改的Fact，则会触发次规则
